...
class JadwalRepository {
    ...
    public function all() {
        $datas = $this->model->with(['matakuliah','ruangan','kelas'])->get();
        return $datas;
    }

    public function allWithKrs(){
        $datas = Jadwal::whereHas('krs',function($q) use($nim){
            $q->where('nim',$nim);
        })->with(['krs','matakuliah','ruangan','kelas','dosen','hari']);
    }
